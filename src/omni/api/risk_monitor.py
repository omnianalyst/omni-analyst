"""PCA-based risk monitor for the carry book.

Computes the carry book's actual factor exposures by eigendecomposing the
return covariance matrix of its held assets. The carry book is designed to be
delta-neutral (long spot + short perp on the same asset). PCA verifies this:
if the first principal component (the market factor) has significant exposure,
the book has drifted from neutrality and carries hidden directional risk.

Returns a simple verdict: delta-neutral (healthy) or factor-exposed (drift).
"""

from __future__ import annotations

from typing import Any
from uuid import UUID

import numpy as np
import pandas as pd
from neutron import App, Router
from neutron.error import unauthorized
from starlette.requests import Request

from omni.api.trading import _resolve_portfolio
from omni.auth import resolve_audience_from_request
from omni.coverage.visibility import visible_claims_cte

LOOKBACK_DAYS = 60


async def _positions(pool, portfolio_id) -> list[dict]:
    rows = await pool.fetch(
        """
        SELECT symbol, market_type, quantity::float8 as quantity
        FROM position
        WHERE portfolio_id = $1 AND quantity != 0
        """,
        portfolio_id,
    )
    return [dict(r) for r in rows]


async def _prices(pool, symbols: list[str], audience: UUID) -> pd.DataFrame:
    """Daily closes for the carry book's assets from the claim store."""
    if not symbols:
        return pd.DataFrame()

    rows = await pool.fetch(
        f"""
        WITH visible_claims AS (
            {visible_claims_cte("$1")}
        )
        SELECT c.event_date,
               split_part(split_part(c.key, '/', 1), ':', 1) AS asset,
               (c.value ->> 'close')::float8 AS close
        FROM visible_claims c
        WHERE c.claim_type = 'price_snapshot'
          AND c.source = 'ccxt'
          AND c.event_date > now() - interval '90 days'
          AND split_part(split_part(c.key, '/', 1), ':', 1) = ANY($2::text[])
        ORDER BY c.event_date
        """,
        audience,
        symbols,
    )
    if not rows:
        return pd.DataFrame()

    df = pd.DataFrame([
        {"date": pd.Timestamp(r["event_date"]).tz_convert(None).normalize(),
         "asset": r["asset"], "close": r["close"]}
        for r in rows if r["close"] is not None
    ])
    if df.empty:
        return pd.DataFrame()

    panel = df.pivot_table(index="date", columns="asset", values="close", aggfunc="last")
    return panel.sort_index().tail(LOOKBACK_DAYS)


def _extract_assets(positions: list[dict]) -> list[str]:
    """Extract base asset symbols from position symbols (BTC/USDC -> BTC)."""
    assets: set[str] = set()
    for pos in positions:
        sym = pos["symbol"].split("/")[0].split(":")[0]
        if sym:
            assets.add(sym)
    return sorted(assets)


def _pca_risk(
    returns: pd.DataFrame,
    positions: list[dict],
    prices: pd.Series,
) -> dict[str, Any]:
    """Eigendecompose the covariance matrix and check for market-factor exposure.

    For a delta-neutral book, the position-weighted exposure to the first
    principal component (market mode) should be near zero. If it's not, the
    book is carrying hidden directional risk.
    """
    assets = returns.columns.tolist()
    if not assets or len(returns) < 20:
        return {
            "verdict": "insufficient_data",
            "detail": f"Need 1+ assets and 20+ days; have {len(assets)} assets, {len(returns)} days",
        }

    position_assets = _extract_assets(positions)
    missing_assets = sorted(set(position_assets) - set(assets))
    if missing_assets:
        return {
            "verdict": "insufficient_data",
            "detail": f"Missing return history for: {', '.join(missing_assets)}",
        }

    return_values = returns.to_numpy(dtype=float)
    if not np.isfinite(return_values).all():
        return {
            "verdict": "insufficient_data",
            "detail": "Return history contains non-finite values",
        }

    return_std = returns.std().to_numpy(dtype=float)
    if np.isclose(return_std, 0.0, atol=1e-12, rtol=0.0).any():
        return {
            "verdict": "insufficient_data",
            "detail": "PCA requires non-zero return variance for every asset",
        }

    marks = prices.reindex(assets).to_numpy(dtype=float)
    if not np.isfinite(marks).all() or np.any(marks <= 0.0):
        return {
            "verdict": "insufficient_data",
            "detail": "Current prices must be finite and positive for every asset",
        }

    cov = returns.cov()
    if not np.isfinite(cov.to_numpy(dtype=float)).all():
        return {
            "verdict": "insufficient_data",
            "detail": "Return covariance is non-finite",
        }
    eigenvalues, eigenvectors = np.linalg.eigh(cov.values)

    # Sort descending (eigh returns ascending)
    order = np.argsort(eigenvalues)[::-1]
    eigenvalues = eigenvalues[order]
    eigenvectors = eigenvectors[:, order]

    # First PC = market factor. Its explained variance share:
    total_var = eigenvalues.sum()
    if not np.isfinite(total_var) or total_var <= 0.0:
        return {
            "verdict": "insufficient_data",
            "detail": "PCA requires positive total return variance",
        }
    pc1_share = float(eigenvalues[0] / total_var)

    # Net quantity and current notional per asset (spot + perp, signed).
    net: dict[str, float] = {a: 0.0 for a in assets}
    notionals: dict[str, float] = {a: 0.0 for a in assets}
    gross_notional = 0.0
    for pos in positions:
        sym = pos["symbol"].split("/")[0].split(":")[0]
        if sym in net:
            qty = float(pos["quantity"])
            if not np.isfinite(qty):
                return {
                    "verdict": "insufficient_data",
                    "detail": "Position quantities must be finite",
                }
            notional = qty * float(prices[sym])
            net[sym] += qty
            notionals[sym] += notional
            gross_notional += abs(notional)

    if not np.isfinite(gross_notional) or np.isclose(
        gross_notional, 0.0, atol=1e-12, rtol=0.0
    ):
        return {
            "verdict": "insufficient_data",
            "detail": "Gross notional must be finite and positive",
        }

    # Normalize factor projection by gross leg notional so paired legs retain a
    # denominator even when their same-asset net exposure is exactly zero.
    notional_vector = np.array([notionals[a] for a in assets])
    pc1_vector = eigenvectors[:, 0]
    pc1_exposure = abs(float(notional_vector @ pc1_vector)) / gross_notional
    net_ratio = abs(sum(notionals.values())) / gross_notional

    if pc1_exposure < 0.05:
        verdict = "delta_neutral"
    elif pc1_exposure < 0.15:
        verdict = "slight_drift"
    else:
        verdict = "factor_exposed"

    return {
        "verdict": verdict,
        "net_ratio": round(net_ratio, 4),
        "pc1_exposure": round(pc1_exposure, 4),
        "pc1_share": round(pc1_share, 3),
        "pc1_label": _pc1_label(pc1_share),
        "positions": {a: round(net[a], 6) for a in assets},
        "notionals": {a: round(notionals[a], 2) for a in assets},
        "gross_notional": round(gross_notional, 2),
        "eigenvalues": [round(float(e), 8) for e in eigenvalues[:5]],
        "n_assets": len(assets),
        "n_days": len(returns),
    }


def _pc1_label(share: float) -> str:
    if share > 0.6:
        return "One factor dominates (typical: assets move together)"
    if share > 0.4:
        return "Moderate common factor"
    return "Diversified (no single dominant factor)"


async def _assess(pool, audience: UUID, portfolio_id: UUID) -> dict[str, Any]:
    """Assess the carry book's risk via PCA."""
    positions = await _positions(pool, portfolio_id)
    if not positions:
        return {"verdict": "flat", "detail": "No open positions"}

    assets = _extract_assets(positions)
    prices = await _prices(pool, assets, audience)

    if prices.empty or len(prices) < 20:
        return {
            "verdict": "insufficient_data",
            "detail": f"Need 20+ days of price history; have {len(prices)}",
            "positions": len(positions),
        }

    returns = prices.pct_change(fill_method=None).dropna()
    result = _pca_risk(returns, positions, prices.iloc[-1])
    result["portfolio_id"] = str(portfolio_id)
    return result


def build_router(app: App) -> Router:
    router = Router()

    @router.get("/scanner/risk")
    async def scanner_risk(request: Request) -> dict:
        audience = resolve_audience_from_request(request)
        if audience is None:
            raise unauthorized("Authentication required")

        pool = app.db.pool
        portfolio_id = await _resolve_portfolio(pool, audience, request.query_params)
        return await _assess(pool, audience, portfolio_id)

    return router


__all__ = ["build_router"]
